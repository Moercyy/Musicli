package mks.moklaussevi.youtubeDownloader;
...
import mks.moklaussevi.youtubeExtractor.VideoMeta;
import mks.moklaussevi.youtubeExtractor.YouTubeExtractor;
import mks.moklaussevi.youtubeExtractor.YtFile;
...
public class DownloadActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Um diese App in "Teilen" anzuzeigen
        if (savedInstanceState == null && Intent.ACTION_SEND.equals(getIntent().getAction())
            ...
            if (ytLink != null
                    && (ytLink.contains("://youtu.be/") || ytLink.contains("youtube.com/watch?v=") /*|| ytLink.contains("www.youtube.com/playlist")*/)) {
                youtubeLink = ytLink;
                // sobald der Link verifiziert wurde, dass es ein  Youtube Link ist
                getYoutubeDownloadUrl(youtubeLink);
            } else {
                //Fehlermeldung falls es kein Link ist
    ...
    private void addButtonToMainLayout(final String videoTitle, final YtFragmentedVideo ytFrVideo) {
        // zeigt die möglichen Downloadformate an (audio, video) sowie die verschiedenen Qualitätsstufen
        String btnText;
        ...
        btn.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                String filename;
                //beschränkt den titel der Datei auf maximal 55 Stellen, mehr geht laut Erfahrung noch nicht
                if (videoTitle.length() > 55) {
                    filename = videoTitle.substring(0, 55);
                } else {
                    filename = videoTitle;
                }
                //ungültige oder nicht verarbeitbare Zeichen ersetzen
                filename = filename.replaceAll("[\\\\><\"|*?%:#/]", "");
                filename += (ytFrVideo.height == -1) ? "" : "-" + ytFrVideo.height + "p";
                String downloadIds = "";
                boolean hideAudioDownloadNotification = false;
                //Formatierungen in die richtigen Buttons
